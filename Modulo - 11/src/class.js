...
class tourOperator extends hotePrice {
  ...
  CalculateSubtotal() {
    this._subtotal = this._booking.reduce(
      (result, { noches, pax, desayuno }) => {
        result += this._RoomPrice[0].price * noches;
        if (pax > 1) result += 40 * (pax - 1);

        if (desayuno) result += 15 * pax;
  ...
  CalculateTotal() {
    this._totalPrice = (
      this._subtotal * this._iva -
      this._subtotal * this._iva * 0.15
    ).toFixed(2);
  }
}
...
class normalClient extends hotePrice {
  ...
  CalculateSubtotal() {
    this._subtotal = this._booking.reduce(
      (result, { tipoHabitacion, noches, pax, desayuno }) => {
  ...
  CalculateTotal() {
    this._totalPrice = (this._subtotal * this._iva).toFixed(2);
  }
}
